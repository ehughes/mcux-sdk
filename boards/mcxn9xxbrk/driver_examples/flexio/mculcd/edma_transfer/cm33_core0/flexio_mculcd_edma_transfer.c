...
static flexio_mculcd_edma_handle_t s_flexioLcdHandle;
...
static status_t DEMO_LcdWriteCommand(void *dbiXferHandle, uint32_t command)
{
    ...
    flexio_mculcd_edma_handle_t *flexioLcdHandle = (flexio_mculcd_edma_handle_t *)dbiXferHandle;
...
static status_t DEMO_LcdWriteMemory(void *dbiXferHandle, uint32_t command, const void *data, uint32_t len_byte)
{
    ...
    status         = FLEXIO_MCULCD_TransferEDMA(flexioLCD, flexioLcdHandle, &xfer);
...
static void DEMO_InitFlexioMcuLcd(void)
{
    ...
    EDMA_Init(DEMO_EDMA, &edmaConfig);
    EDMA_CreateHandle(&s_edmaTxHandle, DEMO_EDMA, DEMO_FLEXIO_TX_DMA_CHANNEL);

#if defined(FSL_FEATURE_EDMA_HAS_CHANNEL_MUX) && FSL_FEATURE_EDMA_HAS_CHANNEL_MUX
    EDMA_SetChannelMux(DEMO_EDMA, DEMO_FLEXIO_TX_DMA_CHANNEL, DEMO_FLEXIO_TX_DMA_REQUEST);
#endif

    /* Create FLEXIO_MCULCD EDMA handle. */
    status = FLEXIO_MCULCD_TransferCreateHandleEDMA(&flexioLcdDev, &s_flexioLcdHandle, DEMO_FLEXIO_TransferCallback,
                                                    NULL, &s_edmaTxHandle, NULL /* RX not used in this example. */
    );
